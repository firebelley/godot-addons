...
func play(): 
	...
	var child = valid_children[random_index] as AudioStreamPlayer2D
	if randomize_pitch:
...
func remove_stream_player(node: AudioStreamPlayer2D):
	node.owner = null
	remove_child(node)
	node.queue_free()
	...
	for i in range(streams.size()):
		...
		if (streams[i] != null):
			...
			if existing_node != null:
				...
				existing_node.name = "AudioStreamPlayer2D%s" % (existing_node.get_index() + 1)
			else:
...
func extract_first_node_with_stream(existing_nodes: Array, resource_path: String) -> AudioStreamPlayer2D:
	var found_node: AudioStreamPlayer2D = null
